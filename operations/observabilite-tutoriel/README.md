...
```bash
./mvnw -pl operations/observabilite-tutoriel quarkus:dev
```
...
```bash
curl -s "http://localhost:8080/tavern/order?drink=stout"
```

## Metriques metier (approche factorisee)

Le service est annote avec `@BusinessTimed`, ce qui produit :
- un timer `tavern.order` (expose comme `tavern_order_seconds_*`)
- un compteur `tavern.order.count` (expose comme `tavern_order_count_total`)

## Tests

```bash
./mvnw -pl operations/observabilite-tutoriel test
```
